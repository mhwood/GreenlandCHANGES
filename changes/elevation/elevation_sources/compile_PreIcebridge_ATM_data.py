...
#######################################################################################
#These are the scripts for finding a list of preicebridge_atm files and their associated links for downloading
def find_preicebridge_atm_dem_files_from_nsidc_portal(GD_object):
    ...
    # this function is from the preicebridge_atm data page
    ...
    short_name = 'BLATM2'
    version = '1'
    time_start = '1993-06-23T00:00:00Z'
    time_end = '2008-10-30T23:59:59Z'
    ...
    for url in urls:
        if url[-len('nadir5seg_50pt'):] == 'nadir5seg_50pt':
            urlParts = url.split('/')
            fileID = urlParts[-1]
            outputFiles.append(fileID)
    ...
    f = open(os.path.join(GD_object.project_folder, GD_object.region_name, 'Elevation','Metadata',GD_object.region_name + ' PreIceBridge ATM Files.csv'),'w')
    f.write(textOutput[:-1])
    f.close()

    return (outputSensors,outputFiles,outputDates,outputUrls)

def find_preicebridge_atm_dem_files_in_domain(GD_object):
    if GD_object.region_name + ' PreIceBridge ATM Files.csv' not in os.listdir(os.path.join(GD_object.project_folder, GD_object.region_name, 'Elevation','Metadata')):
        dem_file_sensors, dem_files, dem_file_dates, dem_file_links = find_preicebridge_atm_dem_files_from_nsidc_portal(GD_object)
    else:
        f=open(os.path.join(GD_object.project_folder, GD_object.region_name, 'Elevation','Metadata',GD_object.region_name + ' PreIceBridge ATM Files.csv'))
        lines=f.read()
        f.close()
        lines=lines.split('\n')
        dem_files=[]
...
#######################################################################################
#These are the scripts for downloading the preicebridge_atm data
...
def download_preicebridge_atm_files(GD_object,dem_file_names, dem_file_dates, dem_file_sensors, dem_file_links):

    if 'PreIceBridge' not in os.listdir(os.path.join(GD_object.data_folder,'Elevation')):
        os.mkdir(os.path.join(GD_object.data_folder,'Elevation','PreIceBridge'))
    if 'Data' not in os.listdir(os.path.join(GD_object.data_folder,'Elevation','PreIceBridge')):
        os.mkdir(os.path.join(GD_object.data_folder,'Elevation','PreIceBridge','Data'))

    dataFolder = os.path.join(GD_object.data_folder,'Elevation','PreIceBridge','Data')
...
#######################################################################################
#These are the scripts for creating the layers in the preicebridge_atm data


def read_valid_points_from_file(filepath):
    f=open(filepath)
    lines = f.read()
    f.close()
    lines=lines.split('\n')
...
def cumulate_preicebridge_atm_dem_field(valid_points,regionX,regionY,dem_grid,count_grid):
...
def get_preicebridge_atm_layers(GD_object,dem_file_names, dem_file_dates, dem_file_sensors):
    ...
    for dd in range(len(dem_dates)):
        ...
        for ff in range(len(date_files[dd])):
            ...
            valid_points = read_valid_points_from_file(os.path.join(GD_object.data_folder, 'Elevation', 'PreIceBridge', 'Data', dem_file_sensor, dem_date,dem_file))
            ...
            dem_grid,count_grid = cumulate_preicebridge_atm_dem_field(valid_points, GD_object.elevation_grid_x,
...
def save_preicebridge_atm_layers(GD_object,dem_layers, dem_dates, dem_decYrs, dem_file_names):
    ...
    output_file = os.path.join(GD_object.project_folder,GD_object.region_name,'Elevation','Data',GD_object.region_name+' PreIceBridge ATM Elevation Grids.nc')
    print('        Outputting data to ' + output_file)

    swath.to_netcdf(output_file)

def save_preicebridge_atm_layers_as_points(GD_object, dem_layers, dem_dates, dem_decYrs, dem_file_name_strings):
    ...
    if len(GD_object.preicebridge_atm_output_file)>2:
        output_file = GD_object.preicebridge_atm_output_file
    else:
        output_file = os.path.join(GD_object.project_folder, GD_object.region_name, 'Elevation', 'Data',
                                   GD_object.region_name + ' PreIceBridge ATM Elevation Points.nc')
...
#######################################################################################
#This is the main routine to process the preicebridge_atm data

def generate_PreIcebridge_ATM_dataset(GD_object):

    message = '    Running compilation for the Operation PreIcebridge ATM data'
    GD_object.output_summary += '\n' + message
    ...
    if GD_object.overwrite_existing_elevation_stacks:
        ...
        if GD_object.region_name+' PreIceBridge ATM Elevation Points.nc' in os.listdir(os.path.join(GD_object.project_folder,GD_object.region_name,'Elevation','Data')):
            continue_with_generation = False
            message = '        PreIceBridge ATM elevation compilation skipped for ' + GD_object.region_name + ' - already done'
            GD_object.output_summary += '\n' + message
            if GD_object.print_main_outputs:
                print(message)
        else:
    ...
    if continue_with_generation:
        ...
        message = '        Finding a list of PreIceBridge ATM files which overlap the domain'
        GD_object.output_summary += '\n' + message
        if GD_object.print_sub_outputs:
            print(message)
        dem_file_names, dem_file_dates, dem_file_sensors, dem_file_links = find_preicebridge_atm_dem_files_in_domain(GD_object)
        print('          Found '+str(len(dem_file_names))+' files')

        if isinstance(GD_object.max_number_of_preicebridge_atm_files,int):
            if len(dem_file_names)>GD_object.max_number_of_preicebridge_atm_files:
                dem_file_names = dem_file_names[:GD_object.max_number_of_preicebridge_atm_files]
                dem_file_dates = dem_file_dates[:GD_object.max_number_of_preicebridge_atm_files]
                dem_file_sensors = dem_file_sensors[:GD_object.max_number_of_preicebridge_atm_files]
                dem_file_links = dem_file_links[:GD_object.max_number_of_preicebridge_atm_files]
        ...
        if GD_object.print_sub_outputs:
            print(message)
        download_preicebridge_atm_files(GD_object,dem_file_names, dem_file_dates, dem_file_sensors, dem_file_links)

        if GD_object.create_elevation_stacks:
            message = '        Resampling the PreIceBridge ATM data onto to the regional domain'
            GD_object.output_summary += '\n' + message
            if GD_object.print_sub_outputs:
                print(message)
            ...
            if GD_object.overwrite_existing_elevation_data:
                ...
                if len(GD_object.preicebridge_atm_output_file) > 2:
                    ...
                    output_file = os.path.join(GD_object.project_folder, GD_object.region_name, 'Elevation', 'Data',
                                               GD_object.region_name + ' PreIceBridge ATM Elevation Grids.nc')
            ...
            if continue_to_stack:
                ...
                # step 3: stack the preicebridge_atm data into layers
                dem_layers, dem_dates, dem_decYrs, dem_filename_strings = get_preicebridge_atm_layers(GD_object,dem_file_names, dem_file_dates, dem_file_sensors)

                if len(dem_layers)>0:

                    # step 4: save the preicebridge_atm layer to an nc file
                    save_preicebridge_atm_layers_as_points(GD_object,dem_layers, dem_dates, dem_decYrs, dem_filename_strings)